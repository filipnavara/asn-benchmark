...
>
  ...
  <xsl:template match="node()[name()]" mode="EncodeImplicitValue" priority="-9">
    <xsl:message terminate="yes">Error, unknown EncodeImplicitValue node [<xsl:copy-of select="."/>]</xsl:message>
  </xsl:template>
  ...
  <xsl:template match="node()[name()]" mode="DecodeImplicitValue" priority="-9">
    <xsl:message terminate="yes">Error, unknown DecodeImplicitValue node [<xsl:copy-of select="."/>]</xsl:message>
  </xsl:template>
    ...
    {<xsl:apply-templates mode="Validate" /><xsl:apply-templates mode="DefaultFieldDef" /><xsl:apply-templates mode="FieldDef" />
      <xsl:if test="*[@defaultDerInit]">
        ...
        static <xsl:value-of select="@name" />()
        {
            ...
            AsnReader reader;<xsl:if test="asn:SequenceOf | asn:SetOf">
...
namespace <xsl:value-of select="@namespace" />
{
  ...
  <xsl:template match="*" mode="EncodeValue" xml:space="default">
    ...
    <xsl:choose>
      ...
      <xsl:when test="@implicitTag">
        <xsl:apply-templates select="." mode="EncodeImplicitValue"><xsl:with-param name="writerName" select="$writerName"/><xsl:with-param name="indent" select="$indent" /></xsl:apply-templates>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="*" mode="Decode" xml:space="default">
    <xsl:choose>
      <xsl:when test="/asn:Choice">
        <xsl:choose>
          ...
          <xsl:when test="@implicitTag" xml:space="preserve">
            <xsl:if test="position() != 1">else </xsl:if>if (tag.HasSameClassAndValue(<xsl:call-template name="ContextTag" />))
            {<xsl:apply-templates select="." mode="DecodeImplicitValue"><xsl:with-param name="readerName" select="'reader'"/><xsl:with-param name="indent" select="'    '"/><xsl:with-param name="name" select="@name"/></xsl:apply-templates>
            }</xsl:when>
      ...
      <xsl:when test="@implicitTag">
        <xsl:apply-templates select="." mode="DecodeImplicitValue">
  ...
  <!-- Types which handle implicit within EncodeSimple -->
  <xsl:template match="asn:AsnType | asn:Boolean | asn:Integer | asn:BitString | asn:NamedBitList | asn:OctetString | asn:ObjectIdentifier | asn:Enumerated | asn:UTF8String | asn:SequenceOf | asn:SetOf | asn:PrintableString | asn:T61String | asn:IA5String | asn:UtcTime | asn:GeneralizedTime | asn:VisibleString | asn:BMPString" mode="EncodeImplicitValue" xml:space="default">
  ...
  <!-- Types which handle implicit within DecodeSimple -->
  <xsl:template match="asn:AsnType | asn:Boolean | asn:Integer | asn:BitString | asn:NamedBitList | asn:OctetString | asn:ObjectIdentifier | asn:Enumerated | asn:UTF8String | asn:SequenceOf | asn:SetOf | asn:PrintableString | asn:T61String | asn:IA5String | asn:UtcTime | asn:GeneralizedTime | asn:VisibleString | asn:BMPString" mode="DecodeImplicitValue" xml:space="default">
    ...
    <xsl:template match="asn:AsnType" mode="FieldDef">
        internal <xsl:value-of select="@typeName"/><xsl:if test="@optional | /asn:Choice">?</xsl:if> <xsl:value-of select="@name" />;</xsl:template>
  ...
  <xsl:template match="asn:AnyValue" mode="FieldDef">
        internal ReadOnlyMemory&lt;byte&gt;<xsl:if test="@optional | /asn:Choice">?</xsl:if> <xsl:value-of select="@name" />;</xsl:template>
  ...
  <xsl:template match="asn:AnyValue" mode="EncodeImplicitValue">
    <xsl:param name="writerName" />
    <xsl:message terminate="yes">IMPLICIT ANY is not yet supported (<xsl:value-of select="@name" />).</xsl:message>
  </xsl:template>
  ...
  <xsl:template match="asn:AnyValue" mode="DecodeSimpleValue" xml:space="default">
    ...
    <xsl:param name="indent" />
    <xsl:param name="name" select="concat('decoded.', @name)"/>
    <xsl:if test="1" xml:space="preserve"><xsl:if test="@universalTagNumber and not(/asn:Choice)">
            ...
            <xsl:value-of select="$indent"/><xsl:value-of select="$name"/> = <xsl:value-of select="$readerName"/>.GetEncodedValue();</xsl:if>
  </xsl:template>
  ...
  <xsl:template match="asn:Boolean" mode="FieldDef">
        internal bool<xsl:if test="@optional | /asn:Choice">?</xsl:if> <xsl:value-of select="@name"/>;</xsl:template>
  ...
  <xsl:template match="asn:Integer[not(@backingType)] | asn:Integer[@backingType = 'BigInteger']" mode="FieldDef">
        internal System.Numerics.BigInteger<xsl:if test="@optional | /asn:Choice">?</xsl:if> <xsl:value-of select="@name" />;</xsl:template>
  
  <xsl:template match="asn:Integer[@backingType = 'byte']" mode="FieldDef">
        internal byte<xsl:if test="@optional | /asn:Choice">?</xsl:if> <xsl:value-of select="@name" />;</xsl:template>

  <xsl:template match="asn:Integer[@backingType = 'int']" mode="FieldDef">
        internal int<xsl:if test="@optional | /asn:Choice">?</xsl:if> <xsl:value-of select="@name" />;</xsl:template>
  ...
  <xsl:template match="asn:Integer" mode="EncodeSimpleValue" xml:space="default">
    <xsl:param name="writerName"/>
    <xsl:param name="indent" />
    <xsl:param name="name" select="@name"/>
    <xsl:variable name="nullable" select="@optional | /asn:Choice"/>
  ...
  <xsl:template match="asn:BitString" mode="DecodeSimpleValue" xml:space="default">
    ...
    <xsl:if test="1" xml:space="preserve">
            ...
            <xsl:value-of select="$indent"/>if (<xsl:value-of select="$readerName"/>.TryGetPrimitiveBitStringValue(<xsl:call-template name="MaybeImplicitCallP"/>out _, out ReadOnlyMemory&lt;byte&gt; tmp<xsl:value-of select="@name"/>))
            <xsl:value-of select="$indent"/>{
            <xsl:value-of select="$indent"/>    <xsl:value-of select="$name"/> = tmp<xsl:value-of select="@name"/>;
            <xsl:value-of select="$indent"/>}
            <xsl:value-of select="$indent"/>else
  ...
  <xsl:template match="asn:NamedBitList" mode="FieldDef">
        internal <xsl:value-of select="@backingType"/><xsl:if test="@optional | /asn:Choice">?</xsl:if> <xsl:value-of select="@name"/>;</xsl:template>
  ...
  <xsl:template match="asn:NamedBitList" mode="DecodeSimpleValue" xml:space="default">
    ...
    <xsl:param name="name" select="concat('decoded.', @name)"/>
    <xsl:if test="1" xml:space="preserve">
            <xsl:value-of select="$indent"/><xsl:value-of select="$name"/> = <xsl:value-of select="$readerName"/>.GetNamedBitListValue&lt;<xsl:value-of select="@backingType"/>&gt;(<xsl:call-template name="MaybeImplicitCall0"/>);</xsl:if>
  </xsl:template>
  ...
  <xsl:template match="asn:OctetString" mode="DecodeSimpleValue" xml:space="default">
    ...
    <xsl:if test="1" xml:space="preserve">
            ...
            <xsl:value-of select="$indent"/>if (<xsl:value-of select="$readerName"/>.TryGetPrimitiveOctetStringBytes(<xsl:call-template name="MaybeImplicitCallP"/>out ReadOnlyMemory&lt;byte&gt; tmp<xsl:value-of select="@name"/>))
            <xsl:value-of select="$indent"/>{
            <xsl:value-of select="$indent"/>    <xsl:value-of select="$name"/> = tmp<xsl:value-of select="@name"/>;
            <xsl:value-of select="$indent"/>}
            <xsl:value-of select="$indent"/>else
  ...
  <xsl:template match="asn:Enumerated" mode="DecodeSimpleValue" xml:space="default">
    ...
    <xsl:param name="name" select="concat('decoded.', @name)"/>
    <xsl:if test="1" xml:space="preserve">
            <xsl:value-of select="$indent"/><xsl:value-of select="$name"/> = <xsl:value-of select="$readerName"/>.GetEnumeratedValue&lt;<xsl:value-of select="@backingType"/>&gt;(<xsl:call-template name="MaybeImplicitCall0"/>);</xsl:if>
  </xsl:template>
  ...
  <xsl:template match="asn:UTF8String | asn:PrintableString | asn:T61String | asn:IA5String | asn:VisibleString | asn:BMPString" mode="DecodeSimpleValue" xml:space="default">
    ...
    <xsl:param name="name" select="concat('decoded.', @name)"/>
    <xsl:if test="1" xml:space="preserve">
            <xsl:value-of select="$indent"/><xsl:value-of select="$name"/> = <xsl:value-of select="$readerName"/>.GetCharacterString(<xsl:call-template name="MaybeImplicitCallP"/>UniversalTagNumber.<xsl:value-of select="local-name()"/>);</xsl:if>
  </xsl:template>
  ...
  <xsl:template match="asn:SequenceOf | asn:SetOf" mode="EncodeSimpleValue" xml:space="default">
    ...
    <xsl:variable name="collNoun">
      <xsl:choose>
        <xsl:when test="asn:SetOf">SetOf</xsl:when>
  ...
  <xsl:template match="asn:SetOf" mode="DefaultTag">Asn1Tag.SetOf</xsl:template>
    ...
    <xsl:template match="asn:UtcTime | asn:GeneralizedTime" mode="FieldDef">
        internal DateTimeOffset<xsl:if test="@optional | /asn:Choice">?</xsl:if> <xsl:value-of select="@name"/>;</xsl:template>
  ...
  <xsl:template match="asn:UtcTime" mode="DecodeSimpleValue" xml:space="default">
    ...
    <xsl:choose>
      <xsl:when test="@twoDigitYearMax" xml:space="preserve">
            <xsl:value-of select="$indent"/><xsl:value-of select="$name"/> = <xsl:value-of select="$readerName"/>.GetUtcTime(<xsl:call-template name="MaybeImplicitCallP"/><xsl:value-of select="@twoDigitYearMax"/>);</xsl:when>
      <xsl:otherwise xml:space="preserve">
            <xsl:value-of select="$indent"/><xsl:value-of select="$name"/> = <xsl:value-of select="$readerName"/>.GetUtcTime(<xsl:call-template name="MaybeImplicitCall0"/>);</xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="asn:GeneralizedTime" mode="DecodeSimpleValue" xml:space="default">
    ...
    <xsl:param name="name" select="concat('decoded.', @name)"/>
    <xsl:if test="1" xml:space="preserve">
            <xsl:value-of select="$indent"/><xsl:value-of select="$name"/> = <xsl:value-of select="$readerName"/>.GetGeneralizedTime(<xsl:call-template name="MaybeImplicitCall0"/>);</xsl:if>
  </xsl:template>
